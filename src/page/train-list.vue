<template>
  <div class="train-list">
    ...
    ></timeCalender>
    <div class="lists center mt20" v-if="trainList&&trainList.length===0">暂无列车信息</div>
    <div class="lists" v-if="trainList&&trainList.length!==0">
      <!-- <scroller lock-x> -->
      <div class="list" v-for="(item,index) in trainList" @click="goBookTicket(item)" :key="index">
        <!-- <flexbox> -->
        <div class="flex p20">
          <div>
            <div class="flex-demo l">
              <span class="font36 orange">{{item.start_time}}</span>
              <br>
              <span>{{item.from_station_name}}</span>
            </div>
          </div>
          <div>
            <div class="flex-demo c1">
              <span>
                {{item.train_no}}
          ...
          <div>
            <div class="flex-demo c2">
              <span class="font36">{{item.arrive_time}}</span>
              <br>
              <span>{{item.to_station_name}}</span>
            </div>
          </div>
        </div>
        ...
        <div class="explain">
          <div class="flex" v-if="item.if_can_by=='Y'">
            ...
            <div class="flexitem" v-if="item.zc1&&item.num1">
              {{item.zc1}}:
              <span>{{item.num1}}</span>
            </div>
          ...
          <div class="orange" v-if="item.if_can_by!=='Y'">暂无余票</div>
        </div>
      </div>
      <!-- </scroller> -->
    </div>
...
export default {
  ...
  data() {
    return {
      menu: [
        { name: "最早出发", icon: "icon-qiehuan" },
        { name: "全部车型", icon: "icon-huochepiao" },
        { name: "综合筛选", icon: "icon-zhengque" },
        { name: "有票优先", icon: "icon-menpiao1" }
      ],
      scrollTop: 0,
      onFetching: false,
      bottomCount: 5,
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      vm.option.sTitle =
        vm.$route.query.FromStation + "-" + vm.$route.query.ToStation;
  ...
  methods: {
    choseDate(date) {
      console.log(date);
      this.leavedate = date;
      this.trainTripArg.FromDate = date;
      this.loadTrainList();
    },
    loadTrainList() {
    ...
    clickbeforedate(date) {
      ...
      this.trainTripArg.FromDate = moment(this.trainTripArg.FromDate)
        .add(-1, "days")
        .format("YYYY-MM-DD");
