  ...
  <div class="train-query">
    <form class="con">
      ...
      <div class="row go-to-city flex v-b1px">
        <div class="flexitem left" @click="eventShowCityList('go')">{{selectCity.goCity}}</div>
        <div class="flexitem">
          <span class="change-icon" @click="changeCity"></span>
        </div>
        <div class="flexitem right" @click="eventShowCityList('to')">{{selectCity.toCity}}</div>
      </div>
      <!--日期-->
      <div class="row flex v-b1px" @click="calendar.show=true">
        <div class="w90">
          ...
          &nbsp;
          <span class="font28">周{{calendar.calendarDay}}</span>
        </div>
        <div class="w10 right">
          <i class="arrow iconfont icon-jinrujiantou1"></i>
        </div>
      </div>
      <!--学生票 只看高铁动车选项-->
      <div class="row flex" style="height: 0.8rem;line-height:0.8rem;">
        <div class="left w40" style="align-self: flex-end;">
          <label class="label" @click="isStudent=!isStudent">
            <input type="radio" :checked="isStudent">
            <span class="text">学生票查询</span>
          </label>
        </div>
        <div class="left w60" style="align-self: flex-end;">
          <label class="label" @click="onlySeeGD=!onlySeeGD">
            <input type="radio" :checked="onlySeeGD">
            <span class="text">只看高铁动车</span>
          </label>
        </div>
      </div>
      <!--搜索按钮-->
      <div class="btn-wrap">
        <button class="btn" @click="goListRouter($event)">开始搜索</button>
        <span
          class="history font26 mr20 col666"
          v-for="(item,index) in historys"
          :@click="selectHistory(item)"
          :key="index"
        >{{item.FromStation}}-{{item.ToStation}}</span>
        <span class="clearHistory font26" v-if="historys.length" @click="clearHistory">清除历史记录</span>
      </div>
    </form>
    <!-- 城市选择弹层 -->
    <my-popup v-show="selectCity.show">
      <trainCity
        :fromToType="selectCity.type"
        @changeCityName="changeThisCity"
        @closePop="closePopup"
      ></trainCity>
    </my-popup>
    <!-- 日历弹层 -->
...
import {
  ...
  getHistory,
  clearHistory
} from "@/assets/js/storage_historySearch"; // localStorage存储
...
export default {
  ...
  data() {
    return {
      menu: [
        ...
        { name: "我的订单", icon: "icon-dingdan", link: "/orderList" }
      ],
      selectCity: {
        show: false,
        hackReset: true,
  ...
  created() {
    ...
    this.calendar.calendarDate =
      queryinfo && queryinfo.FromDate > today ? queryinfo.FromDate : today;
    this.selectCity.goCity =
      queryinfo && queryinfo.FromStation ? queryinfo.FromStation : "北京";
    this.selectCity.toCity =
      queryinfo && queryinfo.ToStation ? queryinfo.ToStation : "杭州";
    this.isStudent =
      queryinfo && queryinfo.isStudent ? queryinfo.isStudent : false;
    this.onlySeeGD = queryinfo && queryinfo.gaoDong ? queryinfo.gaoDong : false;
    this.calendarDayStr();
    this.dateModalData.currentDate = [
      moment(this.calendar.calendarDate).format("YYYY-MM-DD")
    ];
  },
  methods: {
    closePopup() {
    ...
    selectDateEvent(data) {
      ...
      this.calendarDayStr();
      this.dateModalData.startDate = moment(data.startDate).format(
        "YYYY-MM-DD"
      );
      this.dateModalData.currentDate = [
  ...
  .con {
    ...
    .go-to-city {
      ...
      .change-icon {
        ...
        width: 0.6rem;
        height: 0.6rem;
        background: url(../assets/img/icon/change_icon.png) no-repeat center;
